...
class SectionItem extends Component {
    ...
    render() {
        ...
        let image_scale = project.fields.imageScale ? project.fields.imageScale : 1;

        let data = '';

        let data_portrait = '';

        if (project.fields.slug != "home-video") {

            if (project.fields.medias.length > 0) {

                const backgroundLandscape = {
                    backgroundImage: `url(${project.fields.medias[0].fields.file.url})`,
                    transform: 'scale(' + image_scale + ')'
                };

                const backgroundPortrait = {
                    backgroundImage: `url(${project.fields.medias[1].fields.file.url})`,
                    transform: 'scale(' + image_scale + ')'
                };

                switch (project.fields.model) {
                    case 'Screen':
            ...
            const swiperClass = `swiper--video swiper--video-${project.fields.model}`;

            if (!project.fields.medias) {
                ...
                data = <div className={swiperClass}>
                    ...
                    <div className="video video--iframe vimeo">
                        ...
                        <Vimeo
                            ...
                            video={project.fields.embedVimeo}
                            muted={false}
                            autoplay={true}
                            loop={true}
                            showTitle={false}
                ...
                let media = project.fields.medias[0].fields.file;
        ...
        return <div className="swiper-slide">
            <Link onClick={e => { e.preventDefault();  window.location.href = '/projects'}} to='/projects'>{data}{data_portrait}</Link>
        </div>
    }
}
